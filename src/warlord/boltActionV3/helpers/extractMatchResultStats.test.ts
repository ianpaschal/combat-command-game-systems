...
describe('extractMatchResultStats()', () => {
  ...
  it('returns the correct number of points for each player.', () => {
    ...
    const tournamentScoring = { win: 3, draw: 1, loss: 0 };
    const score = calculateMatchResultScore(details, tournamentScoring);
    const [player0Stats, player1Stats] = extractMatchResultStats(details, tournamentScoring);
    expect(player0Stats.points).toEqual(score?.[0]);
    expect(player1Stats.points).toEqual(score?.[1]);
  });

  it('returns no points outside a tournament, where there is no points conversion.', () => {
    const details = {
      winner: 0,
      player0UnitsLost: 3,
      player1UnitsLost: 6,
    ...
    const [player0Stats, player1Stats] = extractMatchResultStats(details);
    expect(player0Stats.points).toEqual(0);
    expect(player1Stats.points).toEqual(0);
  });
